...
class NewsReporterGraph:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            self.route_by_intent,
            {
                ...
                "post_request":   "format_for_linkedin",
        ...
        workflow.add_edge("format_for_linkedin", "post_to_linkedin")
    ...
    def _build_resume_graph(self):
        ...
        sub.add_edge("format_for_linkedin", "post_to_linkedin")
    ...
    def _route_after_confirmation(self, state: AgentState) -> str:
        return "format_for_linkedin" if state.get("confirmed") else "prepare_response"
    ...
    async def await_confirmation(self, state: AgentState) -> AgentState:
        ...
        state["user_response"] = (
            "🔍 **Here's what I found — shall I post this to LinkedIn?**\n\n"
            f"{preview_text}\n"
            "---\n"
        ...
        logger.info("Pausing for user confirmation before LinkedIn post")
        return state
    ...
    async def prepare_response(self, state: AgentState) -> AgentState:
        ...
        if intent == "news_then_post" and not state.get("confirmed") and summaries:
            ...
            if post_success:
                ...
                state["user_response"] = state.get("formatted_content", "Posted successfully")
            else:
                state["user_response"] = (
                    f"News fetched but posting failed: {post_error}\n\n"
                    f"**Summaries:**\n{summaries_text}"
                )
        else:
            state["user_response"] = (
                "I can only help with tech news queries or LinkedIn posts. "
                "Try: 'What's the latest in AI?' or 'Find AI news and post it on LinkedIn'."
            )
