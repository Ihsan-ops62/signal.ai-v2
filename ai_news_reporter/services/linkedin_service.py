...
# Per‑user rate limiting for LinkedIn
...
async def _is_duplicate_content(username: str, content: str) -> bool:
    """Check if the same content was posted by this user in the last hour."""
    if not username:
        return False
    ...
    if doc:
        logger.info("Duplicate content detected for user %s (hash %s)", username, content_hash)
        return True
    return False
...
class LinkedInService:
    ...
    async def _resolve_token(username: str) -> Optional[str]:
        ...
        try:
            from services.oauth_service import get_access_token
    ...
    async def create_post(content: str, access_token: Optional[str] = None,
        ...
        # Check for duplicate content within last hour
        if username and await _is_duplicate_content(username, content):
            return {
                "success": False,
                "error": "Duplicate post: You already published this exact content recently. Please wait or change the content."
            }
        ...
        if result["success"]:
            ...
            # Store content hash for future duplicate checks (will be done in store_results)
        return result
