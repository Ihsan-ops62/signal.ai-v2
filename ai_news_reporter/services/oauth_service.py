...
# ── Encryption
_RAW_KEY = config.TOKEN_ENCRYPTION_KEY
if _RAW_KEY:
    _fernet = Fernet(_RAW_KEY.encode())
else:
    logger.warning("TOKEN_ENCRYPTION_KEY not set — generating a temporary key. Set this in production!")
...
# ── Token Storage (with username normalisation)
...
# ── Per‑user+platform refresh locks
...
async def get_access_token(username: str, platform: str) -> Optional[str]:
    ...
    async with lock:
        ...
        # Return the old token (might still work)
...
async def exchange_linkedin_code(code: str, username: str) -> dict:
    ...
    try:
        ...
        # Fetch person URN to store alongside token
        person_urn = None
        try:
...
async def _refresh_linkedin_token(username: str, refresh_token: str) -> Optional[str]:
    """Attempt to refresh a LinkedIn access token. Returns new token or None."""
    if not LINKEDIN_CLIENT_ID or not LINKEDIN_CLIENT_SECRET:
        return None
    username = username.lower()
    try:
