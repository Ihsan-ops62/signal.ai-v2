...
class TwitterService(BaseSocialService):
    ...
    async def create_post(
        ...
        if username:
            ...
            if not allowed:
                return {
                    "success": False,
                    "error": "Rate limit: Only one tweet per 30 seconds allowed. Please wait."
                }

        if len(content) > 280:
            content = content[:277] + "..."

        result = await self._tweet_with_retry(token, content)
        if result["success"]:
    ...
    async def delete_post(
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                resp = await client.delete(f"{_TWITTER_TWEETS_URL}/{post_id}", headers=headers)
            if resp.status_code == 200:
                deleted = resp.json().get("data", {}).get("deleted", False)
                if deleted:
                    logger.info("[Twitter] Tweet deleted: %s", post_id)
                    return {"success": True, "tweet_id": post_id}
            try:
    ...
    async def _tweet_with_retry(self, access_token: str, content: str, max_attempts: int = 3) -> dict:
        delay = 2.0
    ...
    async def exchange_code(code: str, username: str, code_verifier: str) -> dict:
        ...
        try:
            ...
            from services.oauth.oauth_service import store_token
